...
/// expected type). Distinct from <see cref="Lexer.EnvLexerException"/>, which
/// surfaces syntactic errors in the input itself.
/// </summary>
[Serializable]
public class EnverException : Exception
{
    ...
    /// exception.
    /// </summary>
    public EnverException(string message, Exception inner)
        : base(message, inner)
    {
        Variable = string.Empty;
    }
    ...
    /// </summary>
    public EnverException(string variable, string message)
        : base(message)
    {
        Variable = variable;
    }
    ...
    /// </summary>
    public EnverException(string variable, string message, Exception inner)
        : base(message, inner)
    {
        Variable = variable;
    }
    ...
    /// <summary>
    /// Throws <see cref="EnverException"/> indicating that the named environment
    /// variable was expected but not set.
    /// </summary>
    [DoesNotReturn]
    public static void ThrowMissingEnvironmentVariable(string variable)
    {
        throw new EnverException(variable, $"Environment variable '{variable}' has not been set.");
    }
    ...
    {
        throw new EnverException(variable, $"Environment variable '{variable}' is not valid.");
    }

    /// <summary>
    /// Throws <see cref="EnverException"/> indicating that the same key was
    /// defined more than once in a single parse source.
    /// </summary>
    [DoesNotReturn]
    public static void ThrowDuplicateKey(string variable)
    {
        throw new EnverException(variable, $"Duplicate key '{variable}' encountered in input.");
    }
}
